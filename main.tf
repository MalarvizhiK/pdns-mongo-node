...
data ibm_is_vpc "test_schematics_demo_vpc" {
	name = "malar-vpc"
}
...
resource "ibm_is_subnet" "test_schematics_demo_subnet" {
  name            = "schematics-demo-subnet"
  vpc             = data.ibm_is_vpc.test_schematics_demo_vpc.id
  zone            = "us-south-1"
  ipv4_cidr_block = "10.240.0.0/24"
}
*/

data ibm_is_subnet test_schematics_demo_subnet {
...
resource "ibm_is_instance" "test_schematics_demo_vsi_client" {
  ...
  name           = "schematics-demo-vsi-client"
  image          = data.ibm_is_image.test_schematics_demo_image.id
  profile        = "bx2-16x64"
  resource_group = data.ibm_resource_group.rg.id

  primary_network_interface {
    subnet          = data.ibm_is_subnet.test_schematics_demo_subnet.id
...
resource "ibm_is_instance" "test_schematics_demo_vsi_server" {
  depends_on = [ibm_is_security_group_rule.test_schematics_demo_sg_rule_all_out]
  name           = "schematics-demo-vsi-server"
  image          = data.ibm_is_image.test_schematics_demo_image.id
  profile        = "bx2-16x64"
  resource_group = data.ibm_resource_group.rg.id
...
resource "ibm_resource_instance" "test_schematics_demo_pdns" {
  depends_on        = [data.ibm_is_vpc.test_schematics_demo_vpc]
  name              = "schematics-demo-pdns"
...
resource "ibm_dns_permitted_network" "test_schematics_demo_pdns_permitted_network" {
  ...
  vpc_crn     = data.ibm_is_vpc.test_schematics_demo_vpc.resource_crn
}
